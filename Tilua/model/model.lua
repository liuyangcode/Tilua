...
local is_array = helpers.is_array
local is_string = helpers.is_string
local is_number = function(v) return type(v) == "number" end
local is_scalar = lw_utils.is_scalar
local in_array = lw_utils.in_array
local find = helpers.find
local bind1 = helpers.bind1
local stringx = { strip = helpers.strip, split = split }
-- tablex-compatible helpers
local tablex = {
    find = helpers.find,
    values = function(t)
        local r = {}
    ...
    map = function(fn_or_t, t_or_fn)
        -- support tablex.map(fn, t) and legacy tablex.map(t, fn)
        local fn, t
        if type(fn_or_t) == "function" then
            fn, t = fn_or_t, t_or_fn
        else
            t, fn = fn_or_t, t_or_fn
        end
        local r = {}
        if type(t) ~= "table" then return r end
        if t[1] ~= nil then
    ...
    update = function(dst, src)
        return helpers.extend(dst or {}, src or {})
    end,
    ...
    foreachi = function(t, fn)
        for i, v in ipairs(t or {}) do fn(v, i) end
    end,
}
local pl_utils = {
    bind1 = bind1,
...
-- process-local schema cache (shared within worker)
local _SCHEMA_MEM = {}

local model = class.define()
---s属性初始化
---@protected
function model:properties()
    ...
    -- soft delete: false | true (=deleted_at) | "column_name"
    self.soft_delete = self.soft_delete or false
    -- relations map (optional)
...
---@return model
---@param ctx app
function model:_construct(ctx, name, tablePrefix, connection)
    self:properties()
    ---@type app
    self.ctx = ctx
...
function model:_facade(data)
    ...
    if self.fields then
        ...
        -- O(1) field membership via hash set
        local field_set = self._field_set
        if not field_set then
            field_set = {}
            local vals = tablex.values(fields)
            for _, f in ipairs(vals) do field_set[f] = true end
            -- also allow keys if fields is map name->info
            if type(fields) == "table" and fields[1] == nil then
                for k in pairs(fields) do
                    if type(k) == "string" then field_set[k] = true end
                end
            end
            self._field_set = field_set
        end
        for key, val in pairs(data) do
            if not field_set[key] then
                if self.options.strict then
                    error('data type not valid :[' .. key .. '=' .. tostring(val) .. ']')
                else
                    data[key] = nil
                end
...
function model:delete(options)
    ...
    if soft.is_enabled(self) and not (options and options.force) and not self.options.force then
        local col = soft.column(self)
        local now = (ngx and ngx.localtime and ngx.localtime()) or os.date("%Y-%m-%d %H:%M:%S")
        return self.db:update({ [col] = now }, options)
    end
...
function model:select(options)
    ...
    local resultSet, err = self.db:select(options)
...
function model:_parseOptions(options)
    ...
    if soft.is_enabled(self) then
        options = soft.apply_scope(self, options)
    end
...
---sql查询
---@param sql string
function model:query_bind(sql, params)
    return self.db:query_bind(sql, params or {})
end

function model:execute_bind(sql, params)
    return self.db:execute_bind(sql, params or {})
end
...
function model:get_table_fields_cache(tableName)
    ...
    if _SCHEMA_MEM[cache_key] then
        return _SCHEMA_MEM[cache_key]
    end
    local handler = self:get_db_fields_cache_hanlder()
    if handler and handler.get then
        local ok, val = pcall(handler.get, handler, cache_key)
        if ok and val then
            _SCHEMA_MEM[cache_key] = val
            return val
        end
    end
    return nil
...
function model:cache_table_fields(tableName, fields)
    local cache_key = self:get_table_fields_cache_key(tableName)
    _SCHEMA_MEM[cache_key] = fields
    local handler = self:get_db_fields_cache_hanlder()
    if handler and handler.set then
        pcall(handler.set, handler, cache_key, fields)
    end
end

function model:get_db_fields_cache_hanlder()
    if self.db_fields_cache_hanlder then
        return self.db_fields_cache_hanlder
    end
    local cache_type = self.ctx.config and self.ctx.config.db_fields_cache_type
    if cache_type and self.ctx.cache then
        local ok, h = pcall(function() return self.ctx.cache[cache_type] end)
        if ok then
            self.db_fields_cache_hanlder = h
        end
    end
    return self.db_fields_cache_hanlder
end
...
-------------------------------------------------
-- Convenience query API + hydration (v0.2.9)
-------------------------------------------------

local Result = require("Tilua.model.result")

--- first() alias of find with current options
...
--- find_by({ field = value, ... })
function model:find_by(attrs)
    if type(attrs) ~= "table" then
        return false
    end
    return self:where(attrs):find()
end

--- value(field) – single column of first row
function model:value(field)
    self.options.field = field
    self.options.limit = 1
    local row = self:find()
...
--- pluck(column [, key]) – list of column values; optional keyed by key
function model:pluck(column, key_field)
    local rows = self:select()
    if not rows or type(rows) ~= "table" then
        return {}
    end
    return Result.pluck(rows, column, key_field)
end

--- count already exists; exists()
function model:exists()
    local n = self:count()
    return n and tonumber(n) and tonumber(n) > 0
end

--- Hydrate raw rows into Result.Row objects (optional)
function model:hydrate(rows)
    return Result.hydrate(rows, self)
end

--- select and hydrate
function model:get_list(options)
    local rows = self:select(options)
    if not rows or type(rows) == "string" then
        return rows
    end
    if self.options and self.options.hydrate == false then
        return rows
    end
    if self.ctx and self.ctx.config and self.ctx.config.orm_hydrate then
        return Result.hydrate(rows, self)
    end
    return rows
end

--- chunk(size, callback) – simple batching via limit/offset
function model:chunk(size, callback)
    size = tonumber(size) or 100
    local page = 1
    while true do
        local opts_order = self.options.order
        local rows = self:page(page, size):select()
...
-------------------------------------------------
-- Soft delete & relations (v0.3.0)
-------------------------------------------------

function model:with_trashed()
    self.options.with_trashed = true
    return self
end

function model:only_trashed()
    self.options.only_trashed = true
    return self
end

function model:force_delete(options)
...
function model:restore()
    local soft = require("Tilua.model.soft_delete")
    if not soft.is_enabled(self) then
        return false
    end
    return soft.restore_null(self)
end

function model:hasOne(name, foreign_key, local_key)
    return require("Tilua.model.relation").hasOne(self, name, foreign_key, local_key)
end

function model:hasMany(name, foreign_key, local_key)
    return require("Tilua.model.relation").hasMany(self, name, foreign_key, local_key)
end

function model:belongsTo(name, foreign_key, owner_key)
    return require("Tilua.model.relation").belongsTo(self, name, foreign_key, owner_key)
end

--- relation("posts") using self.relations config
function model:relation(name)
    return require("Tilua.model.relation").load(self, name)
end

--- Eager load relation onto a list of rows
